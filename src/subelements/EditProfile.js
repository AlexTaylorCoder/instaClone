...
function EditProfile() {
  ...
  function handleSubmit(e) {
    ...
    fetch("http://localhost:3001/users/" + userObj.id, {
      ...
      .then((resp) => resp.json())
      .then((resp) => setuserObj(resp))
      .then(() => navigate("/profile/" + userObj.id));

    let match = false;
    if (
      inputs.username !== userObj.username ||
      inputs.picture !== userObj.picture
    ) {
